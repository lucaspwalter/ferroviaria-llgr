<?php
session_start();
header('Content-Type: application/json');
error_reporting(E_ALL);
ini_set('display_errors', 0); // Não exibir erros no output JSON

require_once __DIR__ . '/../../config/database.php';
if (!isset($_SESSION['operador_id'])) {
    echo json_encode(['sucesso' => false, 'mensagem' => 'Usuário não autenticado']);
    exit();
}

$acao = $_POST['acao'] ?? $_GET['acao'] ?? '';

switch ($acao) {
    case 'cadastrar':
        cadastrarSensor($conn);
        break;
    case 'listar':
        listarSensores($conn);
        break;
    case 'buscar':
        buscarSensor($conn);
        break;
    case 'atualizar':
        atualizarSensor($conn);
        break;
    case 'deletar':
        deletarSensor($conn);
        break;
    case 'simular_leitura':
        simularLeitura($conn);
        break;
    default:
        echo json_encode(['sucesso' => false, 'mensagem' => 'Ação inválida']);
}

if (isset($conn) && $conn instanceof mysqli) {
    $conn->close();
}

function cadastrarSensor($conn) {
    try {
        $codigo = trim($_POST['codigo'] ?? '');
        $tipo = $_POST['tipo'] ?? '';
        $localizacao = trim($_POST['localizacao'] ?? '');
        $latitude = $_POST['latitude'] ?? null;
        $longitude = $_POST['longitude'] ?? null;
        $status = $_POST['status'] ?? 'ativo';
        $unidade_medida = trim($_POST['unidade_medida'] ?? '');
        $observacoes = trim($_POST['observacoes'] ?? '');
        
        if (empty($codigo) || empty($tipo) || empty($localizacao)) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Preencha todos os campos obrigatórios']);
            return;
        }
        
        if (strlen($codigo) > 50) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Código deve ter no máximo 50 caracteres']);
            return;
        }
        
        $sqlCheck = "SELECT id FROM sensores WHERE codigo = ?";
        $stmtCheck = $conn->prepare($sqlCheck);
        $stmtCheck->bind_param("s", $codigo);
        $stmtCheck->execute();
        
        if ($stmtCheck->get_result()->num_rows > 0) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Código já cadastrado']);
            return;
        }
        
        $sql = "INSERT INTO sensores (codigo, tipo, localizacao, latitude, longitude, status, unidade_medida, observacoes) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssddsss", $codigo, $tipo, $localizacao, $latitude, $longitude, $status, $unidade_medida, $observacoes);
        
        if ($stmt->execute()) {
            echo json_encode(['sucesso' => true, 'mensagem' => 'Sensor cadastrado com sucesso!', 'id' => $stmt->insert_id]);
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao cadastrar sensor: ' . $conn->error]);
        }
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}

function listarSensores($conn) {
    try {
        $sql = "SELECT * FROM sensores ORDER BY criado_em DESC";
        $result = $conn->query($sql);
        $sensores = [];
        
        while ($row = $result->fetch_assoc()) {
            $sensores[] = $row;
        }
        
        echo json_encode(['sucesso' => true, 'dados' => $sensores]);
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}

function buscarSensor($conn) {
    try {
        $id = $_GET['id'] ?? 0;
        
        $sql = "SELECT * FROM sensores WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($row = $result->fetch_assoc()) {
            echo json_encode(['sucesso' => true, 'dados' => $row]);
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Sensor não encontrado']);
        }
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}

function atualizarSensor($conn) {
    try {
        $id = $_POST['id'] ?? 0;
        $codigo = trim($_POST['codigo'] ?? '');
        $tipo = $_POST['tipo'] ?? '';
        $localizacao = trim($_POST['localizacao'] ?? '');
        $latitude = !empty($_POST['latitude']) ? $_POST['latitude'] : null;
        $longitude = !empty($_POST['longitude']) ? $_POST['longitude'] : null;
        $status = $_POST['status'] ?? 'ativo';
        $unidade_medida = trim($_POST['unidade_medida'] ?? '');
        $observacoes = trim($_POST['observacoes'] ?? '');
        
        if (empty($id)) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'ID não informado']);
            return;
        }
        
        if (empty($codigo) || empty($tipo) || empty($localizacao)) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Preencha todos os campos obrigatórios']);
            return;
        }
        
        $sql = "UPDATE sensores 
                SET codigo=?, tipo=?, localizacao=?, latitude=?, longitude=?, status=?, unidade_medida=?, observacoes=?, atualizado_em=NOW() 
                WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssddsssi", $codigo, $tipo, $localizacao, $latitude, $longitude, $status, $unidade_medida, $observacoes, $id);
        
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                echo json_encode(['sucesso' => true, 'mensagem' => 'Sensor atualizado com sucesso!']);
            } else {
                echo json_encode(['sucesso' => false, 'mensagem' => 'Nenhuma alteração foi feita']);
            }
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao atualizar sensor: ' . $stmt->error]);
        }
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}

function deletarSensor($conn) {
    try {
        $id = $_POST['id'] ?? 0;
        
        $sql = "DELETE FROM sensores WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        
        if ($stmt->execute()) {
            echo json_encode(['sucesso' => true, 'mensagem' => 'Sensor deletado com sucesso!']);
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao deletar sensor']);
        }
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}

function simularLeitura($conn) {
    try {
        $sensor_id = intval($_POST['sensor_id'] ?? $_GET['sensor_id'] ?? 0);

        if ($sensor_id <= 0) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Sensor não informado']);
            return;
        }

        $stmt = $conn->prepare("SELECT tipo FROM sensores WHERE id = ?");
        $stmt->bind_param("i", $sensor_id);
        $stmt->execute();
        $result = $stmt->get_result();

        if (!$sensor = $result->fetch_assoc()) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Sensor não encontrado']);
            return;
        }

        $faixas = [
            'temperatura' => [20, 80],
            'pressao' => [1, 10],
            'velocidade' => [0, 160],
            'proximidade' => [0, 100],
            'vibracao' => [0, 50],
        ];
        [$min, $max] = $faixas[$sensor['tipo']] ?? [0, 100];
        $valor = round($min + (mt_rand() / mt_getrandmax()) * ($max - $min), 2);

        $stmt = $conn->prepare("UPDATE sensores SET valor_atual = ?, ultima_leitura = NOW(), atualizado_em = NOW() WHERE id = ?");
        $stmt->bind_param("di", $valor, $sensor_id);

        if ($stmt->execute()) {
            echo json_encode(['sucesso' => true, 'valor' => $valor]);
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao atualizar leitura']);
        }
    } catch (Exception $e) {
        echo json_encode(['sucesso' => false, 'mensagem' => 'Erro interno: ' . $e->getMessage()]);
    }
}
?>
