...
function buscarPerfil($conn) {
    ...
    $sql = "SELECT id, nome, email, telefone, cpf, data_nascimento, endereco, cidade, estado, created_at as criado_em FROM usuarios WHERE id = ?";
...
function atualizarPerfil($conn) {
    ...
    $sql = "UPDATE usuarios SET nome=?, telefone=?, data_nascimento=?, endereco=?, cidade=?, estado=?, updated_at=NOW() WHERE id=?";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
