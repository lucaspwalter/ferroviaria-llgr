...
function cadastrarRelatorio($conn) {
    ...
    $sql = "INSERT INTO relatorios (tipo, titulo, descricao, periodo_inicio, periodo_fim, gerado_por, formato, dados_json, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'gerado')";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssisss", $tipo, $titulo, $descricao, $periodo_inicio, $periodo_fim, $gerado_por, $formato, $dados_json);
    if ($stmt->execute()) {
...
function gerarDadosRelatorio($conn, $tipo, $periodo_inicio, $periodo_fim) {
    ...
    switch ($tipo) {
        case 'operacional':
            ...
            $sql = "SELECT status, COUNT(*) as total FROM itinerarios WHERE data_partida BETWEEN ? AND ? GROUP BY status";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $periodo_inicio, $periodo_fim);
            $stmt->execute();
        ...
        case 'incidentes':
            ...
            $sql = "SELECT tipo, COUNT(*) as total FROM alertas WHERE criado_em BETWEEN ? AND ? GROUP BY tipo";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $periodo_inicio, $periodo_fim);
            $stmt->execute();
            $result = $stmt->get_result();
