...
function cadastrarItinerario($conn) {
    ...
    $sql = "INSERT INTO itinerarios (codigo, rota_id, trem_id, data_partida, hora_partida, hora_chegada_prevista, status, passageiros_embarcados, observacoes) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("siissssis", $codigo, $rota_id, $trem_id, $data_partida, $hora_partida, $hora_chegada_prevista, $status, $passageiros_embarcados, $observacoes);
    if ($stmt->execute()) {
...
function atualizarItinerario($conn) {
    ...
    $sql = "UPDATE itinerarios SET codigo=?, rota_id=?, trem_id=?, data_partida=?, hora_partida=?, hora_chegada_prevista=?, status=?, passageiros_embarcados=?, observacoes=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("siissssisi", $codigo, $rota_id, $trem_id, $data_partida, $hora_partida, $hora_chegada_prevista, $status, $passageiros_embarcados, $observacoes, $id);
    if ($stmt->execute()) {
