...
function cadastrarTrem($conn) {
    ...
    $sql = "INSERT INTO trens (codigo, modelo, fabricante, ano_fabricacao, capacidade_passageiros, velocidade_maxima, status, km_rodados, observacoes) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssidsdds", $codigo, $modelo, $fabricante, $ano_fabricacao, $capacidade_passageiros, $velocidade_maxima, $status, $km_rodados, $observacoes);
    if ($stmt->execute()) {
...
function atualizarTrem($conn) {
    ...
    $sql = "UPDATE trens SET codigo=?, modelo=?, fabricante=?, ano_fabricacao=?, capacidade_passageiros=?, velocidade_maxima=?, status=?, km_rodados=?, observacoes=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssidsddi", $codigo, $modelo, $fabricante, $ano_fabricacao, $capacidade_passageiros, $velocidade_maxima, $status, $km_rodados, $observacoes, $id);
    if ($stmt->execute()) {
