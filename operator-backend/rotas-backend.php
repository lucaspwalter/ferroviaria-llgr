...
function cadastrarRota($conn) {
    ...
    $sql = "INSERT INTO rotas (codigo, nome, origem, destino, distancia_km, tempo_estimado, 
            status, numero_paradas, paradas, preco_base, observacoes) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssdssisds", $codigo, $nome, $origem, $destino, $distancia_km, $tempo_estimado, 
                      $status, $numero_paradas, $paradas, $preco_base, $observacoes);
...
function atualizarRota($conn) {
    ...
    $sql = "UPDATE rotas 
            ...
            WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssdssisdsi", $codigo, $nome, $origem, $destino, $distancia_km, $tempo_estimado, 
